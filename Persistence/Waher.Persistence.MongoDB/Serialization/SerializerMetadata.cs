...
namespace Waher.Persistence.MongoDB.Serialization
{
	internal sealed class SerializerTypeDescriptor
	{
		...
		public static bool TryCreate(Type Type, out SerializerTypeDescriptor Descriptor)
		{
			...
			foreach (GeneratedTypeMetadata Item in Chain)
			{
				...
				{
					if (!Names.Add(Member.Name) || !Member.IsPublic || !Member.CanRead || !Member.CanWrite)
						continue;

					SerializerMemberDescriptor DescriptorMember = new SerializerMemberDescriptor(Member);
					if (!DescriptorMember.Ignore)
						Members.Add(DescriptorMember);
				}
			}
	...
	internal sealed class SerializerMemberDescriptor
	{
		public SerializerMemberDescriptor(GeneratedMemberMetadata Metadata)
		{
			...
			this.SerializedName = Metadata.Name;
			...
			foreach (object Attribute in Metadata.Attributes)
			{
				switch (Attribute)
				{
					...
					case ShortNameAttribute ShortNameAttribute:
						...
						this.SerializedName = ShortNameAttribute.Name;
						break;
				}
			}
		}
		...
		public bool IsCaseInsensitiveString => this.MemberType == typeof(CaseInsensitiveString);
